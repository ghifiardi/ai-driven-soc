...
selected_page = st.sidebar.selectbox(
    ...
    ["Overview", "Security Operations Funnel", "Alert Review", "Analytics", "System Architecture", "Alerts & Monitoring"],
    key="page_selector"
)
...
def get_real_alerts():
    ...
    try:
        ...
        if len(df) > 0:
            ...
            # Fix the is_anomaly field to handle NA values
...
def security_operations_funnel_page():
    """
    Security Operations Funnel page with vertical orientation
    """
    ...
    with st.spinner("Fetching real-time funnel data from BigQuery..."):
        metrics = get_real_cla_metrics()
        alerts_df = get_real_alerts()
    
    # Sample funnel data - replace with real BigQuery data
    funnel_data = {
        'raw_events': 14622,
        'ada_analyzed': 12428,
        'taa_triaged': 10195,
    ...
    # Create vertical funnel chart
    st.header("🔄 Security Operations Pipeline")
    ...
    with col1:
        ...
        conversion_data = {
            ...
            'Conversion Rate': [
                round((funnel_data['ada_analyzed'] / funnel_data['raw_events']) * 100, 1),
                round((funnel_data['taa_triaged'] / funnel_data['ada_analyzed']) * 100, 1),
                round((funnel_data['cra_contained'] / funnel_data['taa_triaged']) * 100, 1)
            ]
        }
    ...
    with col2:
        ...
        time_data = {
            ...
            'Volume': [funnel_data['raw_events'], funnel_data['ada_analyzed'], 
                      funnel_data['taa_triaged'], funnel_data['cra_contained']]
        }
...
def alert_review_page():
    ...
    with col1:
        # Fix the is_anomaly check to handle NA values
    ...
    if len(alerts_df) > 0:
        ...
        if selected_alert_display:
            ...
            with col1:
                ...
                # Fix the is_anomaly display to handle NA values
                is_anomaly_value = actual_alert['is_anomaly']
                if pd.isna(is_anomaly_value):
                    st.write(f"**Is Anomaly:** Unknown")
                else:
                    st.write(f"**Is Anomaly:** {is_anomaly_value}")
            ...
            with col2:
                ...
                is_anomaly_value = actual_alert['is_anomaly']
                if pd.isna(is_anomaly_value):
                    st.info("🤖 **ADA Detection:** Analysis pending")
                elif is_anomaly_value:
