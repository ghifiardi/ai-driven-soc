...
def generate_chat_response(user_message, selected_alert_data=None):
    """Generate AI chat response with context"""
    
    # Build context from selected alert
    context = ""
    if selected_alert_data is not None:
        context = f"""
Current Alert Context:
- Alert ID: {selected_alert_data.get('alert_id', 'N/A')}
- Classification: {selected_alert_data.get('classification', 'N/A')}
- Severity: {selected_alert_data.get('severity', 'N/A')}
- Confidence: {selected_alert_data.get('confidence_score', 'N/A')}
- Timestamp: {selected_alert_data.get('timestamp', 'N/A')}

"""
    
    # Try Gemini first
    if st.session_state.gemini_model is None:
        st.session_state.gemini_model = get_gemini_client()
    
    if st.session_state.gemini_model:
        try:
            with st.spinner("🤖 AI is thinking..."):
                prompt = f"""{context}
You are a cybersecurity SOC analyst assistant. Provide concise, actionable threat analysis.
...
"""
            ...
            st.warning(f"⚠️ LLM temporarily unavailable, using enhanced analysis...")
    
    # Fallback to enhanced predefined responses
    user_lower = user_message.lower()
    
    if 'investigate' in user_lower or 'how to' in user_lower:
        return f"""🔍 **Investigation Guidance:**

Based on the current alert, I recommend:

1. **Initial Assessment** (5-10 min):
   - Review network flow and data transfer patterns
   - Check threat intelligence for involved IPs/domains
   - Verify if activity aligns with known user behavior

2. **Deep Investigation** (15-30 min):
   - Query SIEM for similar patterns in last 7 days
   - Check endpoint logs for suspicious processes
   - Review firewall and proxy logs

3. **Threat Hunting** (if warranted):
   - Search for IOCs across environment
   - Check for lateral movement indicators
   - Review authentication logs

4. **Documentation**:
   - Record all findings in incident tracker
   - Update alert classification if needed
   - Provide feedback to improve model

Need more specific guidance? Ask about a particular aspect!"""
    
    elif 'threat' in user_lower or 'risk' in user_lower:
        if selected_alert_data is not None:
            ...
            if severity == 'High' or confidence > 0.8:
                ...
                action = "Immediate investigation required within 1 hour"
            elif severity == 'Medium' or confidence > 0.6:
                risk_level = "🟡 **MEDIUM RISK**"
                action = "Investigation required within 4 hours"
            else:
                risk_level = "🟢 **LOW RISK**"
                action = "Monitor and review within 24 hours"
...
**Rationale:**
The model's confidence and severity classification suggest this alert requires attention. Consider the business context and user behavior patterns when making your final determination.

Provide feedback after investigation to help improve future detections!"""
        else:
            return "Please select an alert to get threat assessment context."
    
    elif 'ip' in user_lower or 'address' in user_lower:
        return """🌐 **IP Address Investigation:**

For any suspicious IP addresses:
...
2. **Internal Context:**
   ...
   - Review historical communication patterns
   - Verify geographic location makes sense

3. **Additional Checks:**
   - WHOIS lookup for ownership
...
4. **Action Items:**
   ...
   - Add to watchlist if suspicious"""
    ...
    elif 'mitre' in user_lower or 'att&ck' in user_lower or 'ttp' in user_lower:
        return """📋 **MITRE ATT&CK Mapping:**

Common TTPs for network anomalies:
...
**Recommended Actions:**
1. Map observed behaviors to specific techniques
2. Check for related techniques in the kill chain
3. Review MITRE ATT&CK documentation for detection methods
4. Document TTPs in incident report

Reference: https://attack.mitre.org/"""
    
    else:
        return f"""👋 **AI SOC Assistant Ready!**

I can help you with:
- 🔍 **Investigation guidance** - Steps to analyze this alert
- 🛡️ **Threat assessment** - Risk evaluation and recommendations
- 🌐 **IP/IOC analysis** - Threat intelligence lookup guidance
- 📋 **TTP mapping** - MITRE ATT&CK framework alignment
- ⚡ **Quick actions** - Immediate response recommendations

**Your question:** "{user_message}"

Could you rephrase or be more specific? For example:
- "How should I investigate this alert?"
- "What's the threat level?"
- "How do I check this IP address?"
- "What MITRE TTPs apply here?"

💡 **Tip:** Select an alert above for context-aware responses!"""
...
def alert_review_page():
    ...
    if not alerts_df.empty:
        ...
        # Prepare selectable IDs based on current view with enhanced labels
        if agg_toggle and isinstance(aggregated, pd.DataFrame) and not aggregated.empty:
            # For aggregated view, create descriptive options
            alert_options = {}
            for idx, row in aggregated.iterrows():
                alert_id = str(row['alert_id'])
                label = f"{alert_id[:20]}... ({row['alerts_count']} alerts, {row['max_severity']})"
                alert_options[label] = alert_id
            alert_ids = list(alert_options.values())
            alert_labels = list(alert_options.keys())
        else:
            # For individual view, create options from visible table
            alert_options = {}
            for idx, row in alerts_df.head(50).iterrows():  # Limit to first 50 for performance
                alert_id = str(row['alert_id'])
        ...
        unique_alert_ids = []
        unique_labels = []
        for label, alert_id in zip(alert_labels, alert_ids):
        ...
        # Display selectbox with descriptive labels
        ...
        # Get actual alert_id from selected label
        selected_alert = alert_ids[alert_labels.index(selected_label)] if selected_label else alert_ids[0]
        
        # Store current selection
        st.session_state.last_selected_alert = selected_alert
        
        # Always show alert if one is selected
        if not selected_alert:
            st.warning("⚠️ Please select an alert from the dropdown above to review and provide feedback")
            return
...
def main():
    # NOTE: Auto-refresh disabled to allow uninterrupted chat interaction
    # Users can manually refresh using the "Refresh Alerts" button
