# Goal & Non-Goals
- **Goal**: Delegate the `security-scan` CI job to the upstream orchestrator workflow so installs succeed on repos without packaging metadata, and update docs/artifacts accordingly.
- **Non-Goals**: Change scanner logic, edit the upstream workflow implementation, or modify the local `test` job.

# Perubahan per File
- file: `.github/workflows/security-scan.yml`
  - Lokasi: job `security-scan`
  - Perubahan: Replace bespoke steps with `uses: ghifiardi/proactive-security-orchestrator/.github/workflows/security-scan.yml@main`, pass `working-directory: '.'`, add `secrets: inherit`.
  - Mengapa: Reusing the orchestrator workflow gives us the install-strategy guard and keeps future fixes centralized.
  - Dampak: Less maintenance; ensures scans run on repos lacking `setup.py`/`pyproject.toml`.
- file: `README.md`
  - Lokasi: Quick Start / Troubleshooting
  - Perubahan: Document that CI calls the orchestrator workflow and no longer requires `pip install -e .` in caller repos; note where SARIF results appear.
  - Mengapa: Align user expectations with automation changes.
  - Dampak: Documentation only.
- file: `.agents/progress.md`
  - Lokasi: new Langkah entry
  - Perubahan: Record the workflow delegation work, quick tests, and residual risks.
  - Mengapa: Preserve spec-first traceability.

# Urutan Eksekusi
1. **Edit workflow** – Update `.github/workflows/security-scan.yml` with the `uses:` block, keep `test` job intact.  
   *Uji cepat*: `act -l security-scan` (syntax check) or `act -n security-scan` if available.
2. **Refresh README** – Add CI note + troubleshooting guidance referencing the orchestrator workflow.  
   *Uji cepat*: Markdown preview / lint.
3. **Run regression test** – Execute `pytest tests/test_cli.py -k scan -v` to confirm the CLI path still works.  
4. **Update progress log** – Append Langkah entry summarizing edits and test results.

# Acceptance Criteria
- `security-scan` job now references the upstream workflow with `working-directory` input and inherited secrets.
- README explicitly states that CI uses the orchestrator workflow and highlights the automatic git-install fallback.
- `.agents/progress.md` contains a new Langkah entry covering the change.
- `pytest tests/test_cli.py -k scan -v` passes.
- Next GitHub Actions run shows the upstream workflow logs (“Determine install strategy”), indicating install fallback is active.

# Rollback & Guardrails
- Rollback path: revert `.github/workflows/security-scan.yml` to the previous version (local job) if needed.
- Guardrail: Upstream workflow already defaults to git install, so even if packaging detection fails the job succeeds; local `test` job remains as safety net.

# Risiko Sisa & Mitigasi
- **Upstream drift**: If future changes to the orchestrator workflow introduce regressions, pin to a specific commit.
- **Permission issues**: SARIF upload still needs `security-events: write`; document requirements in README.
- **Developer confusion**: README clarifies that local editable installs aren’t required, reducing friction.
