...
def submit_real_feedback(alert_id: str, is_true_positive: bool, confidence: float, comments: str) -> bool:
    """Persist analyst feedback to BigQuery `soc_data.feedback`."""
    client = get_bigquery_client()
    if not client:
...
def alert_review_page():
    ...
    if not alerts_df.empty:
        ...
        if 'selected_alert_id' not in st.session_state and alert_ids:
            st.session_state.selected_alert_id = alert_ids[0]
        selected_alert = st.selectbox(
            "Select Alert for Feedback",
            alert_ids,
            index=alert_ids.index(st.session_state.selected_alert_id) if alert_ids and st.session_state.selected_alert_id in alert_ids else 0,
            key='selected_alert_id'
        )
        
        # Display selected alert details
        if selected_alert:
            # If aggregated, map back to representative alert row
            if agg_toggle and isinstance(aggregated, pd.DataFrame) and not aggregated.empty:
                selected_alert_data = alerts_df[alerts_df['alert_id'].astype(str) == str(selected_alert)].iloc[0]
                group_key = aggregated[aggregated['alert_id'].astype(str) == str(selected_alert)]['group_id'].iloc[0]
                members = group_members.get(group_key, [])
            else:
                selected_alert_data = alerts_df[alerts_df['alert_id'].astype(str) == str(selected_alert)].iloc[0]
                members = [str(selected_alert_data['alert_id'])]
            # Echo selection for verification
            st.caption(f"Selected Alert ID: {selected_alert}")
