#!/usr/bin/env python3
"""
Enhanced TAA Flash 2.5 Dashboard
Comprehensive monitoring dashboard for the enhanced TAA with Google Flash 2.5 integration
"""
...
def get_bigquery_data():
    ...
    try:
        ...
        # Query for recent alerts processed by enhanced ADA-TAA
        query = """
        SELECT 
            alert_id,
            confidence_score,
...
def get_pubsub_metrics():
    ...
    try:
        ...
        for topic_name in topics:
            topic_path = publisher.topic_path("chronicle-dev-2be9", topic_name)
            try:
                # Get topic info (simplified)
                metrics[topic_name] = {
                    "status": "active",
                    "messages_published": "N/A"  # Would need monitoring API for real metrics
                }
            except Exception:
                metrics[topic_name] = {
                    "status": "inactive",
        ...
        st.error(f"Error fetching Pub/Sub metrics: {e}")
        return {}
...
def main():
    ...
    if not df.empty:
        ...
        st.warning("No data available from BigQuery")
    
    # Pub/Sub Integration Status
    st.header("📡 Pub/Sub Integration Status")
    
    pubsub_metrics = get_pubsub_metrics()
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        containment_status = pubsub_metrics.get('containment-requests', {}).get('status', 'unknown')
        st.metric("Containment Requests", "🟢 Active" if containment_status == 'active' else "🔴 Inactive")
    
    with col2:
        feedback_status = pubsub_metrics.get('taa-feedback', {}).get('status', 'unknown')
        st.metric("TAA Feedback", "🟢 Active" if feedback_status == 'active' else "🔴 Inactive")
    
    with col3:
        reports_status = pubsub_metrics.get('taa-reports', {}).get('status', 'unknown')
        st.metric("TAA Reports", "🟢 Active" if reports_status == 'active' else "🔴 Inactive")
