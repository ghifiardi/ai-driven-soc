#!/bin/bash
# CLA Deployment Script - Using pre-compiled binary wheels when possible
# Run this after logging into the VM

set -e
echo "Starting CLA binary-focused deployment..."
...
echo "Installing dependencies individually with binary preference..."
pip install --only-binary=:all: numpy==1.16.6
pip install --only-binary=:all: pandas==0.25.3
pip install --only-binary=:all: scikit-learn==0.22.2
pip install --only-binary=:all: matplotlib==3.0.3
pip install --only-binary=:all: seaborn==0.9.0
