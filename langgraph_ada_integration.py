...
class LangGraphAnomalyDetectionAgent:
    ...
    def publish_alert_node(self, state: ADAState) -> Dict[str, Any]:
        ...
        try:
            ...
            self.bq_client.insert_rows_json(self.config.BIGQUERY_RESULTS_TABLE_ID, [result_row])
        ...
        async def main_loop():
            ...
            while True:
                try:
                    ...
                    if processed_ids:
                        self.bigquery_client.update_alert_status(alarm_ids=processed_ids, new_status="PROCESSED")
                    if error_ids:
                        self.bigquery_client.update_alert_status(alarm_ids=error_ids, new_status="ERROR")
