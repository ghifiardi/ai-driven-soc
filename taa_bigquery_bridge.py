#!/usr/bin/env python3
"""
TAA BigQuery Bridge Service
===========================

This service bridges the gap between enhanced TAA Pub/Sub output and BigQuery tables
that the dashboard expects. It listens to enhanced TAA Pub/Sub topics and writes
the results to the appropriate BigQuery tables.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('taa_bigquery_bridge.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("TAA-BigQuery-Bridge")

class TAABigQueryBridge:
    """
    Bridge service that writes enhanced TAA results to BigQuery tables
    """
    ...
    def __init__(self):
        ...
        logger.info(f"🚀 TAA BigQuery Bridge initialized")
        ...
        logger.info(f"📊 Writing to TAA state table: {TAA_STATE_TABLE}")
        logger.info(f"📊 Writing to processed ADA alerts: {PROCESSED_ADA_ALERTS_TABLE}")
    ...
    def write_enhanced_result_to_bigquery(self, enhanced_result: Dict[str, Any]) -> bool:
        """Write enhanced TAA result to BigQuery taa_state table"""
        try:
            # Prepare row for taa_state table
            row = {
                "alarmId": enhanced_result.get('alarm_id', 'unknown'),
                "timestamp": datetime.now().isoformat(),
                "events": json.dumps({
                    "original_alert": enhanced_result.get('raw_alert', {}),
                    ...
                    "enhanced_source": enhanced_result.get('enhanced_source', 'enhanced_taa'),
                ...
                "taa_decision": enhanced_result.get('enhanced_classification', 'unknown'),
                "taa_confidence": float(enhanced_result.get('enhanced_confidence', 0.0)),
                "taa_severity": enhanced_result.get('alert_severity', 'LOW'),
                "taa_source_ip": enhanced_result.get('source_ip', 'Unknown'),
                "taa_destination_ip": enhanced_result.get('destination_ip', 'Unknown'),
                "taa_protocol": enhanced_result.get('protocol', 'Unknown'),
                "taa_attack_category": enhanced_result.get('attack_category', 'Network Flow'),
                "taa_enhanced": True
            }
    ...
    def write_comparison_to_bigquery(self, comparison_data: Dict[str, Any]) -> bool:
        """Write comparison data to BigQuery processed_ada_alerts table"""
        try:
            # Prepare row for processed_ada_alerts table
            row = {
                ...
                "raw_alert": json.dumps({
                    ...
                    "source": comparison_data.get('source', 'taa_bigquery_bridge')
                }),
                "enhanced_processing": True,
                "enhanced_threat_score": comparison_data.get('enhanced_threat_score', 0.0),
                "enhanced_is_anomaly": comparison_data.get('enhanced_is_anomaly', False)
            }
    ...
    def log_statistics(self):
        ...
        logger.info(f"📊 Bridge Statistics - Uptime: {uptime}")
    ...
    def start(self):
        ...
        logger.info(f"🚀 Starting TAA BigQuery Bridge Service")
        ...
        logger.info(f"📡 Bridge service listening for messages...")
        ...
        try:
            ...
            logger.info("🛑 TAA BigQuery Bridge Service stopped")
