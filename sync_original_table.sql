-- Continuous Sync: Keep Original Table Updated
-- This procedure can be run periodically to sync new data from taa_comparison to taa_enhanced_results

CREATE OR REPLACE PROCEDURE `chronicle-dev-2be9.gatra_database.sync_to_original_table`()
BEGIN
  -- Insert new records from taa_comparison that don't exist in taa_enhanced_results
  ...
  WHERE tc.created_at >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 1 HOUR)
    AND NOT EXISTS (
