#!/usr/bin/env python3
"""
Integrate TAA Flash 2.5 with existing LangGraph workflow
"""
...
def install_dependencies():
    """Install required dependencies for Flash 2.5"""
...
def update_taa_langgraph():
    """Update the existing TAA LangGraph workflow with Flash 2.5"""
    print("🔄 Updating TAA LangGraph workflow...")
    
    # Read current taa_langgraph_agent.py
    try:
        with open("taa_langgraph_agent.py", "r") as f:
            content = f.read()
    except FileNotFoundError:
        print("❌ taa_langgraph_agent.py not found")
        return False
    
    # Create enhanced version
    enhanced_content = '''from langgraph.graph import StateGraph, END
import logging
import json
...
'''
    ...
    with open("taa_langgraph_enhanced.py", "w") as f:
        f.write(enhanced_content)
    
    print("✅ Enhanced TAA LangGraph workflow created")
    return True

def test_integration():
    """Test the integrated workflow"""
    print("🧪 Testing integrated TAA Flash 2.5 workflow...")
    
    try:
        from taa_langgraph_enhanced import build_taa_workflow, TAAState
        ...
        print(f"LLM Result: {final_state.get('llm_result', {}).get('severity', 'unknown')} severity")
        print(f"Model used: {final_state.get('llm_result', {}).get('llm_model', 'unknown')}")
...
def main():
    ...
    print("🚀 TAA Flash 2.5 Integration")
    print("=" * 40)
    
    # Step 1: Install dependencies
    install_dependencies()
    
    # Step 2: Update TAA LangGraph
    if update_taa_langgraph():
        print("✅ TAA LangGraph updated successfully")
    else:
        print("❌ Failed to update TAA LangGraph")
        return
    ...
    print("\n🎉 TAA Flash 2.5 integration completed successfully!")
    print("\n📋 Next Steps:")
    print("1. Deploy to VM: gcloud compute scp taa_langgraph_enhanced.py app@xdgaisocapp01:~/")
    print("2. Update ADA workflow to use enhanced TAA")
    print("3. Monitor performance and optimize")
    print("\n🚀 Your TAA is now powered by Google Flash 2.5!")
