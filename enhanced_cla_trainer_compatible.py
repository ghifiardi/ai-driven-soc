#!/usr/bin/env python3
"""
Enhanced CLA Trainer - Compatible Version
...
# Use try-except for sklearn imports to handle version issues
try:
...
class EnhancedCLATrainerCompatible:
    """Enhanced CLA Trainer with compatibility fallbacks"""
    ...
    def __init__(self, config_path="enhanced_cla_config.json"):
        try:
            ...
            self.config = {
                ...
                "ensemble_models": ["isolation_forest", "random_forest"],
    ...
    def extract_advanced_features(self, df):
        ...
        if 'raw_alert' in df.columns:
            features['alert_complexity'] = df['raw_alert'].astype(str).str.len().fillna(0)
        ...
        if 'timestamp' in df.columns:
            ...
            features['hour_of_day'] = df['timestamp'].dt.hour.fillna(12)
            features['day_of_week'] = df['timestamp'].dt.dayofweek.fillna(1)
            features['is_weekend'] = (df['timestamp'].dt.dayofweek >= 5).fillna(False).astype(int)
    ...
    def train_ensemble_models(self, X, y):
        ...
        if sklearn_available:
            models_to_train['isolation_forest'] = IsolationForest(
                n_estimators=100,
                contamination=0.1,
                max_samples=0.8,
                random_state=42
            ...
            models_to_train['neural_network'] = MLPClassifier(
            ...
            try:
                if name == 'isolation_forest':
                    ...
                    if max_score > min_score:
                        ...
                        predictions = np.ones(len(scores)) * 0.5
                else:
        ...
        if len(self.models) > 1:
            ...
            # Single model, weight = 1
            self.ensemble_weights = {list(self.models.keys())[0]: 1.0}
                    ...
                    try:
                        if name == 'isolation_forest':
                            ...
                            if max_score > min_score:
                                ...
                                pred = np.ones(len(scores)) * 0.5
                        else:
                            pred = model.predict_proba(X_val)[:, 1]
                        
                        ensemble_pred += weights[name] * pred
                    except Exception as e:
                        print(f"Error in ensemble prediction for {name}: {e}")
                        continue
    ...
    def predict_proba(self, X):
        ...
        for name, model in self.models.items():
            try:
                ...
                print(f"Error in prediction for {name}: {e}")
                continue
...
def main():
    ...
    print("🚀 Starting Enhanced CLA Training (Compatible Version)...")
    
    # Initialize trainer
    trainer = EnhancedCLATrainerCompatible()
