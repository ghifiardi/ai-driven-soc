#!/usr/bin/env python3
"""
SOC Dashboard with Fixed Gemini API Integration
"""
...
def get_gemini_client():
    """Get Gemini client for AI analysis with updated API endpoints"""
    ...
    class GeminiClient:
        def __init__(self, api_key):
            ...
            # Updated API endpoint - using the correct one
            self.base_url = "https://generative-language.googleapis.com/v1"
        
        def enrich_social_media_alert(self, text, user_info):
            """Analyze social media alert using updated Gemini API"""
            try:
                prompt = f"""
                ...
                # Try the correct Gemini model endpoint
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if 'candidates' in result and result['candidates']:
                            ...
                            try:
                                ...
                                return {'success': True, 'enrichment': enrichment, 'model_used': 'gemini-1.5-flash'}
                ...
                with st.spinner("Analyzing with Gemini AI..."):
                    ...
                    if client:
                        ...
                        if result['success']:
                            ...
                            # Show error details
                            st.error(f"❌ Gemini AI Analysis Failed: {result.get('error', 'Unknown error')}")
                            
                            # Provide manual analysis
                            st.info("📋 **Manual Analysis:**")
                            if alert['alert_type'] == "Cyber Threat":
                                st.write("• **Threat Level:** High")
                                st.write("• **Category:** Security Incident")
                                st.write("• **Urgency:** Immediate")
                                st.write("• **Action:** Investigate immediately")
                            elif alert['alert_type'] == "General Complaint":
                                st.write("• **Threat Level:** Low")
                                st.write("• **Category:** Customer Service")
                                st.write("• **Urgency:** Medium")
                                st.write("• **Action:** Forward to customer service")
                            else:
                                st.write("• **Threat Level:** Low")
                                st.write("• **Category:** Social Media")
                                st.write("• **Urgency:** Low")
                                st.write("• **Action:** Monitor")
